...
averageEmp :: Employee -> Real
averageEmp x = toReal(sum list)/toReal(length list)
where
	list = [n \\ n<-:x.salary]

averageEmps :: [Employee] -> Real
averageEmps list = sum(map averageEmp list)

compCalc :: Company Degree -> Real
compCalc comp deg = averageEmps[emp \\ emp<-:comp.employees | isDegree emp.degree == isDegree deg]

highest :: [Company] Degree -> [Company]
highest [x] deg = [x]
highest [x,y:xs] deg
| compCalc x deg > compCalc y deg = highest [x:xs] deg
= highest [y:xs] deg

//Start = highest [cm1,cm2,cm3] BSC


highestAverageSalary :: {Company} Degree -> [Company]
highestAverageSalary arr deg = highest list deg
where
	list = [bruh \\ bruh<-:arr]
...
occurr :: Char {Char} -> Int
occurr le arr = length[wow \\ wow <-:arr | wow==le]

//Start = occurr 'a' {'a','b'}

CountOcc :: {Char} -> {MyPair}
CountOcc arr = {(le, occurr le arr) \\ le<-:arr}
