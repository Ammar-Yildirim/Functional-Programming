...
Level :: (Tree Int) Int  [(Int,Int)] -> (Tree Int)
Level Leaf accum list = Leaf
Level (Node x le ri) accum list
| isMember x (Lister list) = Node ((no2 list x)*accum) (Level le (accum+1) list) (Level ri (accum+1) list)
= Node x (Level le (accum+1) list) (Level ri (accum+1) list)

SwapLevel :: (Tree Int) [(Int,Int)] -> (Tree Int)
SwapLevel tre list = Level tre 1 list
...
isStudent :: UniRelation -> Bool
isStudent Student = True
isStudent Teacher = False

isUtil :: Utility -> Bool
isUtil Electricity = True
isUtil Gas = False

isElte :: University -> Int
isElte ELTE = 1
isElte BME = 2
isElte Corvinus = 3
...
value :: Citizen -> Int
value q 
| isStudent q.rel && isUtil q.utilitySaved = 10000
| isStudent q.rel && (isUtil q.utilitySaved == False) = 5000
| (isStudent q.rel == False) && isUtil q.utilitySaved = 20000
= 10000

elteMoney :: [Citizen] -> Int
elteMoney [] = 0
elteMoney [x:xs] 
| isElte x.uni == 1 = value x + elteMoney xs
= elteMoney xs

bmeMoney :: [Citizen] -> Int
bmeMoney [] = 0
bmeMoney [x:xs] 
| isElte x.uni == 2 = value x + bmeMoney xs
= bmeMoney xs

corvinMoney :: [Citizen] -> Int
corvinMoney [] = 0
corvinMoney [x:xs] 
| isElte x.uni == 3 = value x + corvinMoney xs
= corvinMoney xs
//Start = elteMoney [citizen1,citizen2,citizen3,citizen4,citizen5,citizen6,citizen7,citizen8,citizen9,citizen10,citizen11,citizen12]
/*
elteCount :: [Citizen] -> Int
elteCount list = length[1 \\ y<-list | isElte y.uni == 1 ]
*/

AwardScholarship :: [Citizen] -> [(University, Int, Int)]
AwardScholarship studList = [(ELTE,x,y),(BME,x1,y1),(Corvinus,x2,y2)]
where 
	x = length[1 \\ y<-studList | isElte y.uni == 1 ]
	x1 = length[1 \\ y<-studList | isElte y.uni == 2 ]
	x2 = length[1 \\ y<-studList | isElte y.uni == 3 ]
	y = elteMoney studList
	y1 = bmeMoney studList
	y2 = corvinMoney studList
