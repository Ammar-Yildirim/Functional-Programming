...
//Please write your neptun code here: fa27qm
/*
...
ATM :: Int -> Int
ATM x
= (x/10) + (x rem 10)/5 + (((x rem 10) rem 5) / 2 ) + ((((x rem 10) rem 5) rem 2 ) / 1 )
...
gcd :: Int Int -> Int
gcd x y
| x==0 = abort "Cannot calculate gcd of 0"
| x == y = x
| x>y = gcd(x-y) y
| x<y = gcd x (y-x)

//The first function can be used to find gcd

MyFunGCD :: Int Int -> Int
MyFunGCD x y
| x==0 || y==0 = abort "Cannot calculate gcd of 0" 
| x rem y == 0 || y rem x == 0 = abort "either number is  multiple of the other"
= abort "Cannot calculate gcd of 0" 
