...
average x = toReal(sum[num \\ num<-x.grades]) / toReal(length(x.grades))
...
getBestBScStudent list = (last(sort ls)).id
where
	ls = [stud \\ stud<-list | stud.level == BSc]
...
*/
//Count of a single character
cnt :: String Char -> Int
cnt arr key = length[w \\ w<-:arr | w==key]
...
height BTLeaf accum = accum
height (BTNode le ri) accum = max(height le (accum+1))(height ri (accum+1))

getBTHeight :: BinaryTree -> Int
getBTHeight tree = height tree 1
...
Insert Nil org = Pointer org Nil 
Insert (Pointer str next) org = Pointer str (Insert next org)
...
diagonalSum :: [[Int]] -> Int
diagonalSum list = sum[a!!n \\ a<-list & n<-[0..(length list)-1] ]



largestMat :: {[[Int]]} -> [[Int]]
largestMat arr = fst ((sortBy (\(x,y) (a,b) = y<b ) [(a,diagonalSum a) \\ a<-:arr])!!1)
