...
turner :: Char Int -> Char
turner ch key
| (toInt ch + key)>90 = toChar( (toInt ch) + key - 26)
= toChar(toInt ch + key)


shiftCipher:: String Int -> String
shiftCipher word key = {turner a key \\ a<-:word}
...
distance :: Circle Point -> Real
distance c p = sqrt((x2-x1)^2.0 +(y2-y1)^2.0 )
where
	x1 = c.center.x
	y1 = c.center.y
	x2 = p.x
...
oneCheck :: Circle Point -> Bool
oneCheck c p 
| (distance c p) < toReal(c.radius) = True
= False

check :: Circle {Point} -> Bool
check c arr = and[oneCheck c one \\ one<-:arr]
...
instance + Library
where
	(+) a b = {lib_name = a.lib_name +++ b.lib_name , books = {a \\ a<-list} }
	where
		list = removeDup([ m \\ m<-: b.books] ++ [n \\ n<-: a.books])
...
| list2 > list1 = False
where
	list1 = length[a \\ a<-:ar1]
	list2 = length[a \\ a<-:ar2]

merger :: String String -> String
merger st1 st2 
| isBigger st1 st2 = merge[toString a +++ toString b \\ a<-:st1 & b<-:st2] +++ leftover1
| isBigger st2 st1 = merge[toString a +++ toString b \\ a<-:st1 & b<-:st2] +++ leftover2
= merge[toString a +++ toString b \\ a<-:st1 & b<-:st2]
where
	list1 = [a \\ a<-:st1]
	list2 = [a \\ a<-:st2]
	leftover1 = {a\\a<-(drop (length list2) list1)}
	leftover2 = {a\\a<-(drop (length list1) list2)}
		
instance + {String}
where
	(+) a b = { merger s1 s2 \\ s1<-:a & s2<-:b}
...
height :: (Tree Int) -> Int
height Leaf = 0
height (Node x le ri) = 1 + height le + height ri

AVL_prop_check :: (Tree Int) -> Bool
AVL_prop_check Leaf = True
AVL_prop_check (Node x le ri) 
| abs((height le)-(height ri))<=1 = True && AVL_prop_check le && AVL_prop_check ri
= False
...
toL :: (Tree Int) Int -> Int
toL Leaf k = 0
toL (Node x le ri) k
| x > k = x + toL le k + toL ri k
= toL le k + toL ri k


transform :: (Tree Int) -> (Tree Int)
transform Leaf = Leaf
transform (Node x le ri) = (Node (toL x (Node x le ri)) (transform le) (transform ri))
