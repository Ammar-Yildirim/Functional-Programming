...
where
	toInt p = p.age + p.height + ln
	where
		ln = length[ch \\ ch<-:p.firstName   ]
...
isPrime 0 = False
isPrime 1 = False
isPrime x = length[ num \\ num<-[2..x-1] | x rem num == 0] == 0

//Start = isPrime 3

countLetter :: String -> Bool
countLetter word = isPrime(length list)
where
	list = [b \\ b<-:word | b=='i']

isCountry :: Country -> Bool
isCountry x = countLetter(x.capital)

	
continentsPrimeI :: [Continent] -> [String]
continentsPrimeI x = [a.contName \\ a<-x | length[b\\ b<-:a.countries | isCountry b] >= 1 ]
...
shouldWePlay list gk = length[1 \\ goal<-list | goal.skillLevel >= gk.skillLevel] >=3
...
toDigits x 
| x<10 = numberList.[x]
= toDigits (x/10) +++ "-" +++ numberList.[x rem 10]
*/
//Start = toDigits 125
...
full_words x 
| x<0 = "minus-" +++ full_words (abs x) 
| x<10 = numberList.[x]
= full_words (x/10) +++ "-" +++ numberList.[x rem 10]
...
*/
//YOU HAVE TO ASK THIS THIS HTHIS
:: MaybeInt = Just Int | Nothing

// DEFINITION OF OPERATOR !+! ..... YOUR CODE COMES HERE....
(!+!) infixr 6 :: [Int] Int -> MaybeInt
(!+!) list x				//infix = binary operator, you are defining it from start
| length list >= x+1 = Just (list!!x)	//instance, you are pverloading predefined shit
= Nothing
...
appearance :: {Int} Int -> Bool
appearance arr x = length list >= 2
where
	list = [nums \\ nums<-:arr | nums==x]
	
//Start = appearance {1,2,3,4,5,6,6,6,6,6,6} 6

f7 :: {Int} -> Bool
f7 arr = and[appearance arr num \\ num<-:arr]
