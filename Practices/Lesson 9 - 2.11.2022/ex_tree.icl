...
= task3 le ++ task3 ri		// you dont extract anyhthing if its not odd

extractN :: (Tree Int) -> Int			//this function extracts the value of the left and right digit. If le is leaf it returns -1, if its node it returns thex
extractN Leaf = -1
...
searchT Leaf n = False			//if we are on a leaf there wont be any number, so naturally false
searchT (Node x le ri) n
| x == n = True
=  searchT le n || searchT ri n			//how to keep on and keep together
