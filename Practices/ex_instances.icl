...
instance + String				// were defining plus for the string type
where
   (+) s1 s2 = s1 +++ s2
...
instance - [Int] // minus over a list of integers
where
...
instance + [Int]
where									//You can define instances for only pre-built stuff
    (+) a b = [x+y \\ x<-a & y<-b]		//usually for operators
...
instance * [a] | * a //it doesnt say integer, but we have to make the assumption that *a exists
where				 //so String will not work, because * is not defined for string
	(*) a b = fun a b
	
//Start= [1,2]*[3,4,5,6,0]//[3,8,5,6,0]
// Start= [1,2,3,1,3,12,312] *[2,3]//[2,6,3,1,3,12,312]
//Start :: [Int]			//you have to define for the empty
//Start = [] * []//[]

//size built in for arrays, so you dont have to use l []
...
shorterThan6 uni ={b \\ b <-[y.studentName\\y<-(uni.students)|length[a\\a<-:y.studentName]<6]++[y.name\\y<-(uni.teachers)|length[a\\a<-:y.name]<6]}
																								//strings are arrays of char, so we use<-:
...
= (last (sort x)).uniName				//sort is working because we defined < and =
where x = [y\\y<-:unis]					// we put it into a list
...
where
    (==) Leaf Leaf = True
    (==) (Node x1 l1 r1) (Node x2 l2 r2) = and[x1==x2, l1==l2, r1==r2]	// recursion for l1 and r1
    (==) _ _ = False	//node with leaf wrong, has to be one of up
...
pruneTree :: (Tree a) (a -> Bool) -> (Tree a)	//pred is just a condition
...
insert [x:xs] (z,y)
| length (filter (\(a,b) = a == z) [x:xs]) == 1 = abort "The given key already exists" 	//you check if it exists, if it does you don't add
= [x:xs] ++ [(z,y)]
...
//for list generator plus equal less than are needed, and you can generate records
//3 is the first to be extracted
