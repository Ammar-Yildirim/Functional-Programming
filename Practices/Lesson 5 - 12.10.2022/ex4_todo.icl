...
//Start = not(isoddnrs2 [1..5]) // [True,False,True,False,True]
...
ismult10all x = and(ismult10 x)	//or just rewrite everything for ismult10
...
//powersof32 x = map ((^)3) x		This computes pwers of three, three as a base, its the same as the one above
...
// 0 is whats to be added at the end
...
ins0 :: [[Int]] -> [[Int]]
ins0 x = map ((++)[0]) x	//first

ins0end x = map (\x = x ++ [0]) x
...
//prodf2 x = prod x original, caboce can be writtin with this
...
f2 :: [Int] -> [Int]
f2 x = map ((*)2) (filter ((<) 0) x) // this filters out th elemetns first and the multiplies them by two
...
f5 :: Int [Int] -> [[Int]]
f5 n x = map (rep n) x				//x is omitted
f52 n x = map (\x = rep n x)x 
